...
def ACA2n(a, b, Cin, N, K, valency):
    ...
    x, y = CLA_p(A[0:2*K], B[0:2*K], Cin, 2*K, valency)
    SUM.extend(x)
    cout.append(y)
    for j in range(1, int((N - (2*K)) / K) + 1):
        z, l = CLA_p(A[j * K:(j + 1) * K + K], B[j * K:(j + 1) * K + K], 0, 2*K, valency)
        SUM.extend(z[K:2*K])
        cout.append(l)
...
K = int(input("enter the  K bits for Prediction\n"))

valency = 1

'''curr_dict = os.getcwd()
dir_name = "ACA2_ERROR_ANALYSIS"
...
for i in range(0, iterations):
    ...
    EXACT_SUM = a + b + c
    APPROX_SUM = ACA2n(a, b, c, N, K, valency)
...
print("ACA2n ERROR ANALYSIS\n\n")
print("Parameters :\n")
print("\tN = {0}\n".format(N))
print("\tRESULT BITS = {0}\n".format(K))
print("\tP (PREDICTION BITS) = {0}\n".format(K))
print("\tvalency = {0}\n".format(valency))
...
print("= {0:.3f}  (in %) \n\n".format(int(100 * Acceptance / iterations)))
