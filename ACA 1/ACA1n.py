...
def ACA1_p(a, b, Cin, N, R, P, valency):
    AR = get_bin(a, N)
    BR = get_bin(b, N)
    # Cin = get_bin(C,1)
    # print(int(Cin[0],2))
    ...
    x, y = CLA_p(A[0:1 + P], B[0:1 + P], Cin, 1 + P, valency)
    SUM.extend(x)
    cout.append(y)
    for j in range(1, N - P):
        z, l = CLA_p(A[j :j + 1 + P], B[j :j + 1 + P], 0, 1 + P, valency)
        SUM.extend(z[P:1 + P])
        cout.append(l)
...
P = int(input("enter the  MAXIMUM CARRY PROPOGATION LENGTH.\n"))
valency = 1
R=1
...
for i in range(0, iterations):
    ...
    EXACT_SUM = a + b + c
    APPROX_SUM = ACA1_p(a, b, c, N, 1, P, valency)
...
file.write("ACA1 ERROR ANALYSIS\n\n")
